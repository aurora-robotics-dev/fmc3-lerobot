...
"""Deploy an ACT policy on Fourier GR2 with top + right-wrist RGB cameras only.

This script is adapted from `deploy_gr2_pi0_rgb_right_wrist.py` for the dataset:
...
Unlike Pi0, ACT does not require a language task input.
"""
...
import deploy_gr2_pi0_rgb_right_wrist as right_wrist_base
...
DEFAULT_CHECKPOINT_PATH = (
    "/home/phl/workspace/mymodels/gr2/act/green2yellow/pretrained_model"
)
DEFAULT_DATASET_ROOT = (
    "/home/phl/workspace/dataset/fourier/gr2/muticams/lerobot/"
    "fmc3_gr2_black_capped_bottle_green_to_yellow_lerobot"
)
DEFAULT_RIGHT_WRIST_SERIAL = right_wrist_base.DEFAULT_RIGHT_WRIST_SERIAL
DEFAULT_WINDOW_NAME = "GR2 ACT Deploy | Top + Right Wrist (q/ESC quit, s save)"
...
def _build_act_observation(
    *,
    ...
    observation_np = right_wrist_base._build_visual_observation(
...
def infer_single_action(
    *,
    ...
    raw_action = (
        action_tensor.reshape(-1, action_tensor.shape[-1])[0].detach().cpu().numpy().astype(np.float32)
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Deploy ACT on GR2 with RGB-only top camera + right wrist camera."
    )
    parser.add_argument(
        "--checkpoint-path",
        "--checkpoint",
    ...
    parser.add_argument(
        ...
        help="Dataset root used to resolve the initial pose when omitted.",
    )
    ...
    parser.add_argument("--save-dir", type=str, default="scripts/outputs/deploy_gr2_act_rgb_right_wrist")
...
def run(args: argparse.Namespace) -> None:
    right_wrist_base.WINDOW_NAME = DEFAULT_WINDOW_NAME
    ...
    if need_left_wrist:
        left_wrist_keys = [key for key in visual_keys if "left_wrist" in key.lower()]
        raise ValueError(
            "This deploy script only supports top + right wrist RGB observations. "
            f"Checkpoint expects left wrist keys: {left_wrist_keys}"
        )
    if rgbd_keys:
        raise ValueError(
    ...
    if args.disable_hand_postprocess:
        LOGGER.info("Hand postprocess: disabled")
    else:
        LOGGER.info("Hand postprocess: enabled")
    if not need_right_wrist:
        LOGGER.warning(
            "Checkpoint does not use right wrist camera. Deployment will run with top camera only."
        )

    gui_enabled = right_wrist_base.setup_gui(args)
    ...
    if not gui_enabled and periodic_save_every <= 0:
        ...
        LOGGER.info(
            "GUI unavailable. Saving RGB snapshots every %d steps to %s",
            periodic_save_every,
            save_dir,
        )

    orbbec = right_wrist_base.OrbbecRGBCamera(
    ...
    cam_right = None
    grabber_right: right_wrist_base.ThreadedFrameGrabber | None = None
    client = None
    grabbers: list[right_wrist_base.ThreadedFrameGrabber] = []
    ...
    try:
        ...
        if need_right_wrist:
            if not right_sn:
                ...
                right_sn = right_wrist_base._auto_detect_single_realsense()
        ...
        grabber_top = right_wrist_base.ThreadedFrameGrabber(orbbec.read, "camera_top")
        grabbers = [grabber_top]
        if need_right_wrist:
            grabber_right = right_wrist_base.ThreadedFrameGrabber(
        ...
        if client is not None:
            ...
            init_pose = right_wrist_base._resolve_init_pose(args.dataset_root)
            if args.right_side_only:
                current_state_urdf = base.get_robot_state_urdf(client)
                init_pose = right_wrist_base.apply_left_side_motion_mask(init_pose, current_state_urdf)
        ...
        while True:
            ...
            if not args.disable_right_hand_trigger_coupling:
                raw_action_urdf = right_wrist_base.apply_right_hand_trigger_coupling(raw_action_urdf)
            if args.right_side_only:
                raw_action_urdf = right_wrist_base.apply_left_side_motion_mask(raw_action_urdf, state_full)
            ...
            if gui_enabled:
                should_quit, last_vis_ts = right_wrist_base.render_gui_frame(
                    args=args,
                    top_rgb=top_rgb,
                    right_wrist_rgb=right_rgb,
                ...
                right_wrist_base.save_snapshot(
                    save_dir=save_dir,
                    frame_idx=frame_idx,
                    top_rgb=top_rgb,
                    right_wrist_rgb=right_rgb,
        ...
        if gui_enabled:
            try:
                right_wrist_base.cv2.destroyAllWindows()
                right_wrist_base.cv2.waitKey(1)
            except Exception:
                pass
        if client is not None:
            client.close()
