...
def _depth_u16_to_rgb(depth_u16: np.ndarray) -> np.ndarray:
    ...
    转换流程：
      1. 用 cv2.normalize 将有效深度范围线性映射到 [0, 255]（C++ 实现，比手动 float32 运算快）
      2. 无效像素（depth=0，即传感器未返回深度的区域）置零
    ...
    """
    # 全零深度图（相机尚未就绪或场景无有效深度）直接返回黑图
    if not np.any(depth_u16):
        return np.zeros((*depth_u16.shape, 3), dtype=np.uint8)

    # cv2.normalize 自动找 min/max 并线性映射到 [0, 255]，底层 C++ 实现比 numpy 快
    norm = np.zeros_like(depth_u16, dtype=np.uint8)
    cv2.normalize(depth_u16, norm, 0, 255, cv2.NORM_MINMAX, dtype=cv2.CV_8U, mask=(depth_u16 > 0).astype(np.uint8))

    colored_bgr = cv2.applyColorMap(norm, cv2.COLORMAP_JET)
    # JET colormap 对零值会映射到深红色，手动置黑以区分无效区域
...
def run(args) -> None:
    ...
    try:
        ...
        while True:
            ...
            # 任一相机尚未就绪（首帧前）→ 短暂等待
            if orbbec_frame is None and last_orbbec_id == 0:
                time.sleep(0.001)
                continue

            # 三路都没有新帧 → 跳过本轮推理，避免对同一观测重复计算
            if orbbec_frame is None and left_result is None and right_result is None:
                time.sleep(0.001)
                continue

            # 部分相机有新帧、部分没有 → 用上一次的帧补齐（get_latest 兜底）
            if orbbec_frame is None:
                orbbec_frame = grabber_orbbec.get_latest()
            if left_result is None:
                left_result = grabber_left.get_latest()
            if right_result is None:
                right_result = grabber_right.get_latest()
            ...
            # 关闭腕部深度：用全零图替代，保持模型输入结构不变
            lw_depth_rgb = np.zeros_like(lw_depth_rgb)
            rw_depth_rgb = np.zeros_like(rw_depth_rgb)
            ...
            if not args.dry_run:
                ...
                if first_action and args.transition_time_s > 0:
                    # 首帧：线性插值平滑过渡到目标位置
                    smooth_transition_move_cmd(client, action_urdf)
                    policy.reset()  # 过渡后清空 action chunk 缓存
                    first_action = False
                else:
