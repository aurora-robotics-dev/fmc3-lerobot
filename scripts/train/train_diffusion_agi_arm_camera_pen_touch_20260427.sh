...
RUN_NAME="diffusion_camera_pen_touch_clean_del_52_376_h16_a8_full"
JOB_NAME="${RUN_NAME}"
CONDA_ENV="lerobot-pi0"

STEPS=160000  # about 4.9 epochs for this cleaned dataset with batch_size=8
BATCH_SIZE=8
SAVE_FREQ=5000
LOG_FREQ=50

WANDB_ENABLE=true
WANDB_PROJECT="agi_arm_diffusion"
...
# Example:
#   ./scripts/train/train_diffusion_agi_arm_pick_and_place.sh --policy.crop_shape='[96,96]'
...
validate_setup() {
  if [[ "${POLICY_TYPE}" != "diffusion" ]]; then
    echo "This script is configured for diffusion training. Got POLICY_TYPE='${POLICY_TYPE}'." >&2
    exit 1
  fi
  ...
  if (( N_ACTION_STEPS > HORIZON - N_OBS_STEPS + 1 )); then
    echo "Invalid diffusion rollout setup: n_action_steps=${N_ACTION_STEPS}, horizon=${HORIZON}, n_obs_steps=${N_OBS_STEPS}." >&2
    echo "Require: n_action_steps <= horizon - n_obs_steps + 1" >&2
    exit 1
  fi
...
build_train_command() {
  ...
  add_train_arg "policy.horizon" "${HORIZON}"
  add_train_arg "policy.n_action_steps" "${N_ACTION_STEPS}"
  add_train_arg "policy.vision_backbone" "${VISION_BACKBONE}"
  add_train_arg "policy.use_group_norm" "${USE_GROUP_NORM}"
  add_train_arg "policy.use_separate_rgb_encoder_per_camera" "${USE_SEPARATE_RGB_ENCODER_PER_CAMERA}"
...
print_summary() {
  echo "=========================================="
  echo "Diffusion training summary"
  echo "=========================================="
  ...
  echo "Diffusion rollout   : n_obs=${N_OBS_STEPS}, horizon=${HORIZON}, n_action=${N_ACTION_STEPS}"
