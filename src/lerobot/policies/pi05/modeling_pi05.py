...
class PI05Pytorch(nn.Module):  # see openpi `PI0Pytorch`
    ...
    def embed_prefix(
        ...
        """Embed images, optional tactile maps, and language tokens."""
        embs = []
        pad_masks = []
        att_masks = []
        ...
        if tactile_data is not None and self.config.use_tactile:
            for tactile_map in tactile_data:
                ...
                def tactile_embed_func(tactile_map):
                    tactile_tokens = self.tactile_encoder(tactile_map)
                    return self.tactile_proj(tactile_tokens)

                tactile_emb = self._apply_checkpoint(tactile_embed_func, tactile_map)
                bsize, n_tactile_tokens = tactile_emb.shape[:2]
                ...
                pad_masks.append(
                    torch.ones(
                        ...
                        n_tactile_tokens,
                        dtype=torch.bool,
                        device=tactile_emb.device,
                    )
                )
                att_masks += [0] * n_tactile_tokens
...
class PI05Policy(PreTrainedPolicy):
    ...
    def _extract_tactile_data(self, batch: dict[str, Tensor]) -> list[Tensor] | None:
        """Return enabled tactile tensors in deterministic left/right order."""
        ...
        for key in tactile_keys:
            if key not in batch:
                continue
            tactile_tensor = batch[key]
            if tactile_tensor.dtype != torch.float32:
                tactile_tensor = tactile_tensor.to(torch.float32)
            if device is not None and tactile_tensor.device != device:
                tactile_tensor = tactile_tensor.to(device)
            tactile_data.append(tactile_tensor)

        return tactile_data or None
